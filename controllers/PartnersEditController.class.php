...
class PartnersEditController extends ModuleController {
	...
	{
		$partner_id = AppContext::get_request()->get_getint('partner_id');

		SessionData::add_data('test', 'salut');

		echo '<pre>';
		var_dump(PartnersService::protect_session('test'));
		var_dump(AppContext::get_session());
		echo '</pre>';
		die();

		/*if($_SESSION['partner'] != PartnersService::session_protect('test')){
			AppContext::get_response()->redirect(PATH_TO_ROOT.'/partners/manager');
		}*/
	...
	private function get_partner()
	{
		...
		$row = $this->partner = PersistenceContext::get_querier()->select_single_row_query('SELECT * FROM '.PartnersSetup::$partners_table.' WHERE id=:id', array('id' => $partner_id));
